...
<body>
				...
				<div class="row">
					<div class="col-12">
						...
						<div id="nickCategory" class="row">
							<div id="nickspaceDiv" class="col-4 brbrbr" value="${info.nickname}"></div>

							<div id="nickname_1" class="col-4 userName">
								<h3 id="user_nickname">${info.nickname}</h3>
							</div>

							<div id="nick_corrDiv" class="col-4 nickCorr">
								<img id="nick_corrBtn" type="button" class="nickCorrBtn" src="images/nickCorr.png">
							</div>
						</div>
						<!-- 로고 밑 닉네임 끝 -->
					</div>
									...
									<div id="ex_select" class="col-2">
										...
										<button id="rankgraph<%=i%>" class="rankgraphBtn col-2">
											<div class="text-center move"
												value="<%=exerciseList.get(i).getEx_name()%>">
						...
						<div class="row">
							<div class="col-8">
								<div class="card flex-fill table22">
									...
									<table id="feedbacktable" class="myTable table hover my-0 ">
										<tbody>
											<tr>
												<th colspan="3" class="weightHead"><h5>Feedback History</h5></th>
											</tr>
											<tr>
												<th>Date</th>
												<th>P.T</th>
											...
											<tr>
												...
												<td class="d-none d-xl-table-cell"><span><%=feedbackList.get(i).getEx_name()%></span></td>
												<td><a
													href="feedDetail.do?feed_number=<%=feedbackList.get(i).getFeed_num()%>">내용보기</a></td>
											</tr>
											<% } %>
										</tbody>
									</table>
									...
									<div class="card flex-fill table22">
										<table class="table table-hover my-0 weightTable">
											<thead class="weightHead">
												<tr>
													<th colspan="2"><h5>나의 목표</h5></th>
												</tr>
											</thead>
											<tbody>
												<tr>
													<th>시작 몸무게</th>
													<td><span id="weight_start">${info.weight_start}</span>
														<span>Kg</span></td>
												</tr>
												<tr>
													<th>현재 몸무게</th>
													<td><span id="weight_now">${info.weight_now}</span> <span>Kg</span>
													</td>
												</tr>
												<tr>
													<th>목표 몸무게</th>
													<td><span id="weight_target">${info.weight_target}</span>
														<span>Kg</span></td>
												</tr>
											</tbody>
										</table>
	...
	<script type="text/javascript">
		...
		$(document).on('click', '#nick_corrBtn', ()=> {
			...
			var nickname = $('#nickspaceDiv').attr('value')
			
			$('#nick_corrBtn').remove()
			$('#nickname_1').remove()
			
			$('#nickspaceDiv').attr('class', 'col-8 brbrbr')
			$('#nickspaceDiv').append('<input id="user_nick_corr" type="text" placeholder="' + nickname + '"/>')
			
			$('#nick_corrDiv').append('<button id="nick_correct" class="userbtn rounded">변경</button>')
		...
		$(document).on('click', '#nick_correct_cancel', ()=> {
			...
			$('#nickspaceDiv').attr('class', 'col-4 brbrbr')
			$('#nickspaceDiv').after('<div id="nickname_1" class="col-4 userName"></div>')
			$('#nickname_1').append('<h3 id="user_nickname">' + nickname + '</h3>')
			
			$('#nick_corrDiv').append('<img id="nick_corrBtn" type="button" class="nickCorrBtn" src="images/nickCorr.png">')
		})
		...
		$(document).on('click', '#nick_correct', ()=> {
			...
			$.ajax({
				...
				success: function(res) {
					...
					$('#nickspaceDiv').attr('class', 'col-4 brbrbr')
					$('#nickspaceDiv').attr('value', nickname)
