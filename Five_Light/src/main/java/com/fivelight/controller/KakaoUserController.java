...
public class KakaoUserController {
	...
	public String kakaoCallback(String code, Model model,  HttpSession session) throws IOException {  // Data를 리턴해주는 컨트롤러 함수		
		...
		HttpEntity<MultiValueMap<String,String>> kakaoTokenRequest = new HttpEntity<>(params,headers);  // << 바디값과 헤드값을 가진 값이 된다		
		...
		// HttpHeaders와 HttpBody 하나의 오브젝트에 담기
		HttpEntity<MultiValueMap<String,String>> kakaoProfileRequest2 = new HttpEntity<>(headers2);  // << 바디값과 헤드값을 가진 값이 된다
		...
		System.out.println("우리 사이트 유저 네임 : " + kakaoProfile.getKakao_account().getEmail() + "_" + kakaoProfile.getId());
